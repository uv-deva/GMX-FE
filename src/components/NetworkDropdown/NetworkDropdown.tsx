  ...
  return (
    <>
      <button className="transparent">
        {icon ? (
          <img className="network-dropdown-icon" src={icon} alt={""} />
        ) : (
          <span className="flex h-20 w-20 items-center justify-center rounded-full text-white">
            {selectorLabel.slice(0, 1)}
          </span>
        )}
      </button>
...
function NetworkMenuItems({ networkOptions, selectorLabel }) {
  ...
  console.log({ networkOptions, selectorLabel, active });
