...
export function SubaccountContextProvider({ children }: PropsWithChildren) {
  ...
  const [defaultExecutionFee, defaultNetworkFee] = useMemo(() => {
    ...
    const applyFactorVal = applyFactor(gasLimits.estimatedGasFeeBaseAmount, gasLimits.estimatedFeeMultiplierFactor);
    const getFactorByChainIdVal = getFactorByChainId(chainId);

    const approxNetworkGasLimit =
      applyFactor(
        applyFactorVal,
        getFactorByChainIdVal
      ) +
